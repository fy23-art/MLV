...
We estimate the transition probability P between states using the second-order assumption  P(St+1|St,St-1). The model includes two terminal states: S{win, S} (Serving team wins) and S{win, R} (Receiving team wins). The Sideout Probability is the expected value of reaching the receiving-team terminal state: WPRec(S) = P(Receiving Team Wins | Current State = S). For attackers, the Win Probability Added (WPA) is WP = WPafter-WPbefore
